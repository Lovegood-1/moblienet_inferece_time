...
class MobileNets_con(nn.Module):
...
def mobilenet_con(num_classes, large_img, **kwargs):
    ...
    """
    model = MobileNets_con(num_classes, large_img, **kwargs)
    if use_cuda:
        model = model.cuda()
    return model
if __name__ == '__main__':
    from torchsummary import  summary
    net1 = mobilenet(100,False).cuda()
    inputs = torch.ones((8,3,32,32))
